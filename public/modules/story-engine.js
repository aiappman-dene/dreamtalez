...
import { pick, formatName } from './utils.js';
// =============================================================================
...
export function findInterestMatchedWorld(child = {}) {
...
export function findQuickWishMatchedWorld(wish, child = {}) {
 ...
 */
export function pickSuitableWorld(child = {}) {
  const age = Number(child.age) || 5;
...
export function pickRandomSuitableWorld(child = {}) {
  const age = Number(child.age) || 5;
  const gender = (child.gender || "").toLowerCase();
  const keys = Object.keys(themeWorlds);
...
export function generateCharacter(name, gender) {
  const type =
...
export function buildSeriesContinuationContext(childName, seriesData = {}) {
  const series = seriesData[childName];
...
export function siblingRelation(sibling, mainGender) {
...
export function buildEmergencyFallbackStory(childName = "A Little One") {
...
export function buildSafeFallbackWishHope(name, wish) {
...
export function buildSafeProceduralQuickStory(child = {}, world = null, siblings = []) {
...
export function resolveWorldKey(world) {
  if (!world) return "";
...
export function buildOfflineIdeaArc(name, companion, customIdea, goal, flyingSceneKey = "") {
...
export function buildProceduralTitle(name, { mode = "random", customIdea = "", worldKey = "", world = null } = {}) {
...
export function generateQuickStory(child = {}, world = null, siblings = [], attempt = 1) {
  const rawName = child.name || "a little one";
  const name = formatName(rawName);
  const age = Number(child.age) || 5;
